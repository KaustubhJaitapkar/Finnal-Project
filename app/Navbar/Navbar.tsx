...
import Logo from '@/app/assets/HackMate.svg'

function RoleChooser() {
    const [open, setOpen] = React.useState(false);
    return (
        <div className="relative">
            <button onClick={() => setOpen(o => !o)} className="px-4 py-2 bg-primary text-white rounded hidden lg:inline-block">Get Started</button>
            {open && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 border rounded shadow z-50">
                    <button onClick={() => (window.location.href = '/login?role=participant')} className="w-full text-left p-3 border-b">Participant</button>
                    <button onClick={() => (window.location.href = '/login?role=organizer')} className="w-full text-left p-3">Organizer</button>
                </div>
            )}
        </div>
    )
                ...
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                    <Link href={"/"} className="flex items-center space-x-3 rtl:space-x-reverse" >
                        <div className=' w-10 h-10 grid place-items-center p-1 '>
                            <Image src={Logo} alt="HackMate" width={20} height={20} className=' w-full h-full object-cover ' />
                        </div>
                        
                        <span className="self-center text-lg lg:text-2xl font-semibold whitespace-nowrap dark:text-white">HackMate</span>
                    </Link>
                    ...
                    { status==='unauthenticated' &&
                        ...
                        {/* Role chooser for desktop Get Started */}
                        <RoleChooser />
                    ...
                    <Sheet>
                        ...
                        <SheetContent side="right" className="w-[300px] bg-background">
...

                            {
                                ...
                                <div className=' w-full flex flex-col justify-center items-center gap-2 mb-2 '>
                                    ...
                                    <Link href={`/dashboard/my-applications`} className="w-full flex justify-start items-center gap-4 border border-gray-600 dark:border-gray-800 rounded-md p-4 ">
                                        <span> <MdDashboard /> </span>
                                        <span> Application status </span>
                                    </Link>

                                    <Link href={`/dashboard/requests`} className="w-full flex justify-start items-center gap-4 border border-gray-600 dark:border-gray-800 rounded-md p-4 ">
                                        <span> <MdDashboard /> </span>
                        ...
                        <div className=' w-full flex flex-col gap-2 items-center justify-center'>
                            <div className=' w-full flex items-center justify-center gap-2 '>
                                <Link href="https://github.com/harsh3dev/HackMate" target="_blank" rel="noopener noreferrer">
