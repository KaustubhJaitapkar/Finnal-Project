...
const Page = () => {
  ...
  const platforms = [
    ...
    { name: "Our Hackathons", logo: null },
  ]

  const [ourHackathons, setOurHackathons] = useState<any[]>([]);
  ...
  // Fetch organizer-posted hackathons separately
  useEffect(() => {
    const fetchOur = async () => {
      try {
        const res = await axios.get('/api/hackathons/owned');
        if (res?.data?.hackathons) setOurHackathons(res.data.hackathons);
      } catch (err) {
        console.error('Failed to fetch our hackathons', err);
      }
    }
    fetchOur();
  }, []);
  ...
  return (
    <div className='min-h-screen w-full text-black dark:bg-background bg-background dark:text-white mb-20 relative'>
      ...
      <div className='w-full mt-16 flex flex-col justify-start items-start gap-5'>
        ...
        {
          selectedPlatform === "Our Hackathons" &&
          <div className='w-full min-h-[50vh] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 flex-wrap px-10 '>
            {
              ourHackathons.length === 0 ? (
                <div className="col-span-full text-center py-10">
                  <p className="text-lg text-gray-500">No organizer hackathons yet</p>
                </div>
              ) : (
                ourHackathons.map((entry:any) => (
                  <Posts
                    key={entry.id}
                    id={entry.id}
                    title={entry.title || entry.hackathonName || 'Untitled'}
                    url={`/organizer/hackathons/${entry.id}`}
                    logo={entry.logo || ''}
                    platform='organizer'
                    mode={entry.hackathonMode || entry.mode || ''}
                    location={entry.location || entry.venue || ''}
                    status={entry.status || ''}
                    description={entry.description}
                    regDate={entry.regDate}
                    memberCount={entry.memberCount}
                    applicationsCount={entry.applications ? entry.applications.length : 0}
                  />
                ))
              )
            }
