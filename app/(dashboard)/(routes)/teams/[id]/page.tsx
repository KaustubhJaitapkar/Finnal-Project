...
export default function Page({ params }: { params: { id: string } }) {
    ...
    const applied = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!resume && !resumeUrl) missing.push('Resume');

        if (missing.length > 0) {
            toast.error('Please provide: ' + missing.join(', '));
            console.warn('Cannot submit application, missing fields:', missing);
            return;
        }

        console.log("Applying with LinkedIn:", linkedinUrl, "GitHub:", githubUrl, "and Resume:", resume?.name || resumeUrl);
        ...
        // Log formData keys for debugging before sending
        try {
            const keys: string[] = [];
            // formData.keys() is an iterator
            for (const k of (formData as any).keys()) keys.push(k as string);
            console.log('FormData keys to send:', keys.join(', '));
        } catch (err) {
            console.warn('Could not enumerate FormData keys', err);
        }
        ...
        try {
            ...
            const serverMessage = error?.response?.data?.error || error?.message || 'Error applying';
            ...
            <div className='--outerbox rounded-md border border-gray-700 w-full dark:border-gray-400 max-w-[90%] text-text dark:text-text my-14 p-10 bg-gradient-to-tr from-violet-300 via-violet-200 to-slate-100 dark:from-violet-600 dark:via-indigo-900 dark:to-slate-900 '>
                ...
                <div className='w-full flex justify-between items-start gap-10 mx-auto my-2'>
                    <div className='h-full w-full flex-nowrap lg:col-span-3 mr-2 '>
                        ...
                        {userData?.id === user?.id ?
                            <Button onClick={() => router.push(`/dashboard/requests?postId=${params.id}`)} className='bg-primary font-medium dark:font-bold transition-colors ease-in-out w-full'>
                                View Applications
                            </Button>
                            :
                            <ApplyComponent
