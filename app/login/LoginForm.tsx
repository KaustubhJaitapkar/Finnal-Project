...
export default function LoginForm() {
    ...
    useEffect(()=>{
		// if already authenticated and role param present, redirect straight to role landing
		if (status === 'authenticated' && roleParam) {
			if (roleParam === 'organizer') {
				router.push('/dashboard/organizer');
				return;
			}
			// default participant landing
			router.push(DEFAULT_LOGIN_REDIRECT);
		}
		if (error) {
			toast.error(error, {
		...
		<div className="flex h-screen w-full items-center justify-center ">
			<Card className=" w-[90%] sm:w-full max-w-lg backdrop-blur-lg bg-background dark:bg-background border-gray-800/50 dark:border-gray-400/30 ">
				<CardHeader className="sm:space-y-1 text-left sm:text-center px-8 py-4 sm:p-6 ">
						<CardTitle className="text-2xl font-bold">{roleParam === 'organizer' ? 'Organizer Sign In' : roleParam === 'participant' ? 'Participant Sign In' : 'Log In'}</CardTitle>
						<CardDescription>{roleParam === 'organizer' ? "Sign in to manage your hackathons and review applicants." : roleParam === 'participant' ? "Sign in to apply to teams and manage your applications." : "It's good to see you back here!"}</CardDescription>
				</CardHeader>
				...
				<CardFooter className="text-center gap-2 w-full flex justify-center items-center text-sm text-gray-500">
					...
					<Link href={roleParam ? `/signup?role=${encodeURIComponent(roleParam)}` : '/signup'} className="font-medium text-blue-600 dark:text-cyan-500 hover:underline" prefetch={false}>
						Sign Up
					</Link>
				</CardFooter>
			</Card>
